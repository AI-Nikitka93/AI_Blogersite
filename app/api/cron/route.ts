...
import { publishPostToTelegram, buildPublicPostUrl, getPublicPostUrlBlockReason } from "../../../src/lib/telegram";
...
async function persistAndPublishPost(args: {
  ...
  if (args.supabase) {
    try {
      await (args.supabase.from("posts") as any)
        .update({
        ...
        for (const orphan of orphanedPosts) {
          ...
          if (!publicUrlBlockReason) {
            const telegramResult = await publishPostToTelegram({
              post: orphan as unknown as import("../../../src/lib/agent").MiroPost,
              postId: orphan.id,
