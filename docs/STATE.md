...
Новый актуальный сдвиг от `2026-05-22 14:15`: после deploy proof на `e14f64a` и свежих subagent-аудитов добавлен следующий local hardening slice. Public reader/RSS больше не показывают три known weak historical posts (`921bc906-85f3-4164-a6c4-ff1a66e77992`, `b4b379db-437c-48fd-a30e-023c52b5b927`, `6c90bb36-41e3-4112-b7c0-c5c727714f0a`) до явного repair/regeneration решения. Public quality gate режет fallback self-report phrases вроде `Опорный источник`, `Мировая запись нужна`, `Практическая ценность записи`, а также смешанный формат `Источник фиксирует ...` с длинным raw English fact. Source ranking теперь получает publication-potential score: research/release/result/event signals усиливаются, а promo/customer-story/legal/celebrity/local-culture/where-to-watch сигналы штрафуются. HackerNews дополнительно блокирует law/crime/actor/police/lawsuit/manipulated-by-AI сюжеты, чтобы `tech_world` не протаскивал celebrity crime story как AI news. GitHub cron trigger теперь различает свежий честный category cooldown (`fresh_cooldown_idle`) и слабый active-slot skip (`missed_publish_slot`): cooldown считается benign только при `publish_freshness=pass`, `reader_visibility=pass` и свежем reader-visible посте. Full `npm run check` зеленый; live source audit после правки дал `31 ok`, `1 transient MLB News timeout`, `0 stale`.

Новый актуальный сдвиг от `2026-05-22 13:45`: после production proof и read-only subagent-аудитов закрыт и задеплоен следующий hardening slice. Same-story corroboration больше не считает соседние RSS-посты подтверждением той же истории только из-за общего домена/пути (`openai.com/index`, `mlb.com/news`): hostname убран из story-token comparison, boilerplate tokens добавлены в stop-list, а регрессии покрывают реальные формы OpenAI News и MLB News. Non-market `editorial_fallback` теперь разрешен только для repair-класса: русская локализация или fact-focus/mixed RSS fix; `thin article body` и `opinion detached` больше не превращаются в deterministic filler. Telegram publish path теперь перед `sendMessage` делает bounded public URL check и возвращает `status=skipped`, если detail URL уже 404/500/недоступен. Full `npm run check` зеленый; live source audit дважды имел `stale=0` и по одному transient timeout на разных RSS, так что это source-network noise, не стабильный code regression. Commit `e14f64a` прошел GitHub `CI` run `26283567324`; `CD` workflow_run `26283609148` прошел build/deploy/Vercel smoke. Live `/`, `/archive`, `/feed.xml`, `/api/health` вернули `200`; health `status=ok`, `publish_freshness=pass`, `reader_visibility=pass`. Production previews на задеплоенном коде подтвердили честные `skipped` для слабых `tech_world`, `sports` и `world`, то есть filler не вернулся.
...
- Новый local cron trigger split не должен скрывать слабые skipped slots: только свежий category cooldown при зеленом health и свежем reader-visible посте становится `fresh_cooldown_idle`; thin/weak/generic quality skips остаются `missed_publish_slot` и должны валить workflow на YAML fail step.
- Public reader/RSS now blocks known weak historical posts `921bc906-85f3-4164-a6c4-ff1a66e77992`, `b4b379db-437c-48fd-a30e-023c52b5b927`, and `6c90bb36-41e3-4112-b7c0-c5c727714f0a`; this is a visibility quarantine, not a Supabase data repair.
- Source ranking publication-potential scoring and the law/crime HackerNews filter are local code coverage until CI/CD deploy proof confirms the production bundle includes them.
...
- Cleanup hygiene: `artifacts/verification/` теперь добавлен в `.gitignore` как generated local evidence; `scripts/query-db-real.ts` и `scripts/smoke.ts` теперь дополнительно исключены из git/Vercel deploy surface, потому что читают `.env.local` и требуют отдельного hardening перед tracking. `VERCEL_TOKEN` в GitHub Actions обновлен 2026-05-22, ручной `CD` run `26282355687` прошел pull/build/deploy/smoke; последующий автоматический `CD` run `26283609148` также прошел после commit `e14f64a`.
...
- После следующего deploy проверить новый Telegram public URL gate: если detail page скрыта или 404 до отправки, канал должен получить `status=skipped`, а не ссылку-призрак.
- После следующего deploy проверить, что non-market fallback не публикует thin/detached filler, а только ремонтирует локализацию или смешанные RSS-факты.
- После следующего deploy проверить, что source-ranking больше не завышает corroboration за счет соседних RSS items с тем же доменом.
