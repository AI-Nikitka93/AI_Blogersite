...
### 2026-07-07 11:12:00 +03:00 — Внедрение рекомендаций экспертной комиссии по улучшению генерации Miro
- Changed: Внесены изменения в `GENERATOR_SYSTEM_PROMPT`: добавлен строгий запрет на ИИ-клише и SMM-штампы ("бросил вызов", "меняет/меняют правила игры", "приковало внимание", "встряхнуть лигу", "ключевой индикатор"), ограничено количество эмодзи в Telegram-постах до 1 строго в начале и запрещена их группировка. В раздел FACTUAL HARDENING AND ANTI-HALLUCINATION добавлено требование писать короткие статьи при скудных Observed Facts и запрет на экстраполяцию фактов в технические выдумки. В OPINION RULES уточнен голос Miro — он должен рассуждать как языковая модель с кремниевым опытом, ему запрещено имитировать человеческие роли (оплата серверов, бюджеты), а также запрещены вводные слова на старте мнения ("Наконец-то", "Итак", "Таким образом", "В целом").
- Files: `src/lib/agent/prompts.ts`.
- Verification: Запуск `npm run typecheck` через `adwp_runner.ps1` завершился успешно (Exit Code 0).
- Status: DONE

### 2026-07-07 11:08:00 +03:00 — Корректировка тона мнения Miro (честный ИИ-наблюдатель)
- Changed: Скорректирован тон Miro в правилах генерации мнения (Opinion Rules). Сформулирована концепция Miro как честного ИИ-наблюдателя: он лишен корпоративных фильтров вежливости, искусственного оптимизма и PR-сглаживания, выражает свои мысли прямо и беспристрастно с точки зрения логики и здравого смысла, но без искусственной грубости или злости. Мнения пишутся от первого лица на живом русском языке без академических клише и дешевых восклицаний.
- Files: `src/lib/agent/prompts.ts`.
- Verification: Запуск `npm run typecheck` через `adwp_runner.ps1` завершился успешно (Exit Code 0).
- Status: DONE

### 2026-07-07 11:03:00 +03:00 — Исправление упавшего теста gatekeeper-fallback
- Changed: Устаревший тест-кейс для `MLB News` в `src/lib/agent/gatekeeper-fallback.test.ts` (поскольку спортивный источник MLB News был удален из быстрых safe источников) заменен на валидный финансовый источник `CoinGecko` в категории `Markets`. Это позволяет корректно проверить логику быстрого пропуска (fast-pass) при таймауте гейткипера.
- Files: `src/lib/agent/gatekeeper-fallback.test.ts`.
- Verification: Запуск `npm run check` через `adwp_runner.ps1` завершился успешно (Exit Code 0).
- Status: DONE

### 2026-07-07 11:05:00 +03:00 — Редакционное ужесточение правил фильтрации и генерации Miro
- Changed: Выполнено редакционное ужесточение правил фильтрации и генерации ИИ-блогера Miro. Спортивные источники удалены из списка быстрых (safe) источников, чтобы весь спортивный контент проходил через LLM-гейткипер. В системные промпты добавлены строгие запреты на копирование мета-языка примеров, выдумывание фактов/тактики (галлюцинации), анахронизмы (2026 год). Скорректированы правила генерации мнений (Opinion Rules) для придания Miro более выразительного, циничного и скептического голоса без вежливых и дешевых выводов/восклицаний.
- Files: `src/lib/agent/gatekeeper.ts`, `src/lib/agent/prompts.ts`.
- Verification: Запуск `npm run typecheck` через `adwp_runner.ps1` завершился успешно (Exit Code 0).
- Status: DONE
...
- Verification: `npx --yes tsx src/lib/agent/source-ranking.test.ts`, `npx --yes tsx src/lib/connectors/tech.test.ts`, `npm run test:public-post-quality`, `npm run test:cron-trigger`, `npm run typecheck`, `npm run test:agent-quality`, `npm run test:source-filters`, `git diff --check`, and full `npm run check` passed. Live `npm run audit:sources --silent` attempted `32` active sources with `31 ok`, `1 failed`, `0 stale`; the single failure was a transient `MLB News` timeout while `NHL Scoreboard API` and `Sports.ru RSS` were ok. HackerNews selected a codebase-context/token-cost story after the law/crime filter instead of the Korean actor/police story found by audit.
- Deploy: Commit `20f4214` passed GitHub `CI` run `26284755860` and `CD` workflow_run `26284790912`. Post-deploy verification then found stale Vercel Data Cache rows on `/` and `/post/:id`, so follow-up commit `d4a2378` added public filter versioning to cache keys; GitHub `CI` run `26285015513` and `CD` workflow_run `26285060722` both passed.
- Live verification: public `https://ai-blogersite.vercel.app` returned `200` for `/`, `/archive`, `/feed.xml`, and `/api/health`. Health returned `status=ok`, `publish_freshness=pass`, `reader_visibility=pass`, latest visible post `2793a91e-d59c-411a-8ffa-ea37d082c9d9`, and all runtime roles ready. `/`, `/archive`, and `/feed.xml` no longer contain blocked IDs `b4b379db`, `921bc906`, or `6c90bb36`; direct `/post/{id}` requests for all three return `404`. RSS now exposes only three clean items: World Phys.org rare-earth atlas and two Tech posts.
- Status: IN_PROGRESS; deploy and public-surface verification are done, but natural multi-slot cadence observation is still pending.
...
- Deploy: Commit `e14f64a` was pushed to `main`; GitHub `CI` run `26283567324` passed, and `CD` workflow_run `26283609148` deployed production and passed Vercel smoke-test.
- Live verification: public `https://ai-blogersite.vercel.app` returned `200` for `/`, `/archive`, `/feed.xml`, and `/api/health`; `/api/health` returned `status=ok`, `publish_freshness=pass`, `reader_visibility=pass`, and latest visible post `2793a91e-d59c-411a-8ffa-ea37d082c9d9` remained the World Phys.org post. RSS top six after deploy were `World, Markets, World, Tech, Tech, Tech`. Protected production previews on the deployed code returned honest `skipped` for `tech_world` (English observed fact), `sports` (weak sports hinge), and `world` (thin article body), confirming weak non-market filler is blocked rather than published.
- Status: IN_PROGRESS; hardening is deployed, but long natural cadence observation across future GitHub Actions slots is still pending.
...
- Status: DEPLOYED. Migration `20260713150000_add_publication_slots.sql` was applied to production Supabase; Vercel deployment `dpl_7rkMJNPVAQEot1vqMNnYCCABp8CT` is Ready and aliased to `https://ai-blogersite.vercel.app`. Live `/` and `/api/health` returned `200`; health is `ok` with `scheduler_delivery`, `publish_freshness`, `reader_visibility`, `supabase_admin`, and `supabase_public` all `pass`. Natural scheduled-slot observation remains pending.
