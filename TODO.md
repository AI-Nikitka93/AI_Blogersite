...
- [ ] Fix: refresh or replace the GitHub Actions `VERCEL_TOKEN` secret; CD run `25971356126` failed at `vercel pull` with `The token provided via --token argument is not valid`.
- [ ] Verify: rerun `Miro Cron Trigger` after commit `30c6f4f`; the latest inspected scheduled run before that commit (`25970348513`) returned `missed_publish_slot` from `quality gate blocked English observed fact in Russian mode` while `/api/health` freshness still passed.
