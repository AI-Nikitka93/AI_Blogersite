...
4. [ ] Запуск субагента QA-судьи (QA_Judge) для запуска скрипта через `adwp_runner.ps1` и проверки результатов. (Координатор -> QA_Judge)
5. [ ] Верификация результатов и отчёт пользователю. (Координатор)
