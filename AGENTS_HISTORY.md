...
## 2026-07-11 18:15:00
**Описание работы:**
Устранена проблема ложных сбоев (Exit Code 1) в GitHub Actions при плановом пропуске публикаций (cooldowns, duplicates, quiet windows). Выполнены следующие доработки:
1. В `scripts/trigger-cron.sh` расширена логика `is_category_cooldown_reason` и `is_benign_skip_reason`, чтобы любые типы редакционных пропусков постов корректно классифицировались как `benign_skip=true` (мягкий пропуск).
2. Параметр `fresh_cooldown_idle_hours` увеличен до 9 часов для покрытия максимальных кулдаунов (например, 8 часов для категории Sports).
3. Из файла `.github/workflows/cron.yml` полностью удален жесткий шаг `Fail missed publish slot`, который принудительно обрушал билд с кодом 1 при пропусках. Теперь регулярный пропуск слота выполняется с кодом 0 (зеленый статус), а в логах выводится предупреждение. Технические и инфраструктурные ошибки (сеть, API, HTTP non-200) по-прежнему приводят к аварийному завершению с кодом 1.
...
**Статус:** DONE. Тесты `npm run test:cron-trigger` и `npm run typecheck` пройдены успешно (Exit Code 0).
